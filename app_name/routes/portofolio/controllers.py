...
def get_portfolio_unapproved():
    ...
    try:
        ...
        create_log(f"{currentUser_role} dengan email:{currentUser_email} melakukan get all unnapproved portofolio")
        return success_data(message="Berhasil", data=all_portofolio)
...
def get_myportfolio():
    ...
    approved = int(approved) if approved else 1
    limit  = int(limit) if limit else 10
    offset  = (int(offset) - 1) * limit if offset else 0
    ...
    try:
        ...
        values = (user_id,approved,limit,offset,)
        result = Database().get_data(query,values)
        ...
        create_log(f"{user_role} dengan email:{currentUser_email} melakukan get myportofolio")
        return success_data(message="Berhasil", data=all_portofolio)
...
def update_portfolio(id):  
    ...
    try:  
        ...
        values = (
            ...
            filename_thumbnail, filename1, filename2, filename3 ,id
        )
...
def approve_portfolio(id):  
    ...
    try:  
        ...
        if result[0]["approved"] == 1:
            return defined_error(f"portofolio id:{id} sudah di approved",'Bad Request', 400)
