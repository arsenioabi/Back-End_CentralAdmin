...
def create_event():
    ...
    try:
        ...
        dataRequired = [
            ...
            "poster", "deskripsi", "link_conference"
        ]
        ...
        # validate data
        error_validator_list = event_validator(nama,nama_pemateri,link_conference)
...
def request_create_event():
    ...
    try:
        ...
        # validate data
        error_validator_list = event_validator(nama, nama_pemateri, link_conference, contact_whatsapp)
        if len(error_validator_list) > 0 :
            return defined_error(error_validator_list,'Bad Request', 400)
        ...
        def send_email_verification():
            send_email(
                penerima="taqiyyaghazi@gmail.com", 
...
def update_by_id(id):
    ...
    try:
        ...
        dataRequired = [
            "nama", "tanggal", "waktu_mulai", "waktu_berakhir", "nama_pemateri", "deskripsi",
            "poster","deskripsi", "link_conference", "password"
        ]
        ...
        nama = dataInput["nama"].strip()
        tanggal = dataInput["tanggal"].strip()
        waktu_mulai = dataInput["waktu_mulai"].strip()
        waktu_berakhir = dataInput["waktu_berakhir"].strip()
        nama_pemateri = dataInput["nama_pemateri"].strip()
        poster = dataInput["poster"]
        deskripsi = dataInput["deskripsi"].strip()
        link_conference = dataInput["link_conference"].strip()
        ...
        values = (
            ...
            poster_filename,deskripsi,
            link_conference,password, id,  
        )
        Database().update_data(query, values)
           
        create_log(f"{user_role} dengan email:{user_email} berhasil mengupdate event dengan id:{id}")

        # Send success response
        return success(message=f"Berhasil mengupdate event dengan id:{id}")
    except Exception as e:
        return bad_request(str(e))
...
def update_by_kode(kode):
    ...
    try:
        ...
        values = (
            nama, tanggal, waktu_mulai, waktu_berakhir, nama_pemateri,
            poster_filename,deskripsi,
            link_conference,password, kode,  
        )
        Database().update_data(query, values)
           
        create_log(f"{user_role} dengan email:{user_email} berhasil mengupdate event dengan kode:{kode}")

        # Send success response
        return success(message=f"Berhasil mengupdate event dengan kode:{kode}")
    except Exception as e:
        return bad_request(str(e))
...
def delete_by_id(id):
    ...
    try:
        ...
        values = (id,)
        Database().update_data(query, values)        
        create_log(f"{user_role} dengan email:{user_email} berhasil menghapus event dengan id:{id}")
        # Send success response
        return success(message=f"Berhasil menghapus event dengan id:{id}")
    except Exception as e:
        return bad_request(str(e))
...
def delete_by_kode(kode):
    ...
    try:
        ...
        values = (kode,)
        Database().update_data(query, values)        
        create_log(f"{user_role} dengan email:{user_email} berhasil menghapus event dengan kode:{kode}")
        # Send success response
        return success(message=f"Berhasil menghapus event dengan kode:{kode}")
    except Exception as e:
        return bad_request(str(e))
...
def publisher(action,kode):
    ...
    query = "SELECT email, nama FROM event WHERE kode=%s"
    values = (kode, )
    result = Database().get_data(query, values)
    if len(result) == 0:
    ...
    if(email_narasumber != ""):
        ...
        if action == "publish":
            link_form_kehadiran = f"https://centralai.id/event/kehadiran/{kode}"
